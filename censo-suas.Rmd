...
hash_sha1_dadosgerais <- digest(
  ...
  algo = "sha1",       # Define o algoritmo
  serialize = FALSE    # IMPORTANTE: hash o arquivo bruto, não um objeto R
)
...
HASH - SHA-1 CRAS 2024 - Dados Gerais e RH:
```{r cras-sha1, echo=FALSE}

cat(hash_sha1_dadosgerais)
cat(hash_sha1_rh)
...
modelo <- lm(qtd_cras ~ qtd_cras_rh, data = dados)
...
ggplot(dados, aes(x = qtd_cras_rh, y = qtd_cras)) +
  ...
  labs(
    ...
    subtitle = paste("Intercept (a):",coeficientes[1], "(b):",coeficientes[2]),
    y = "Qtd. CRAS",
    x = "Qtd. Profissionais CRAS"
  )
...
```
```{r cras-mu_qtdprofissionais_X_qtdCRAS, echo=FALSE}
...
ggplot(data = dados, aes(x = qtd_cras_rh, y = qtd_cras)) +
  geom_point() +
  labs(
    title = "Qtd. Profissionais por Qtd. CRAS (Municípios)",
    subtitle = paste("Correlação:",cor_qtdcras_qtdcrasrh),
    y = "Qtd. CRAS",
    x = "Qtd. Profissionais CRAS"
  )
...
```
```{r cras-q01, echo=FALSE}
  
dados <- CRAS_2024_DADOSGERAIS %>%
          select(c(NU_IDENTIFICADOR,q1)) %>%
          group_by(q1) %>%
          summarise(valor = n(), .groups = "drop")

# Gráfico de barras
ggplot(dados, aes(x = q1, y = valor)) +
  geom_col() +
  geom_text(aes(label = formatarNumeroInteiro(valor)), vjust = -0.5, size = 2.5) +
  labs(title = "Capacidade Referenciada dos CRAS",
       x = "Capacidade Referenciada",
       y = "Valor")
